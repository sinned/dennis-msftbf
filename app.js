...
bot.dialog('/', [
    function (session) {
        ...
        //mixpanel.track('start_conversation');

        var card = new builder.HeroCard(session)
            .title('Dennis Built a Bot')
            .text('Hi I am Dennis\'s bot.')
            .images([
                 builder.CardImage.create(session, 'https://c1.staticflickr.com/1/7/10128776_763d3af89c.jpg')
            ]);
        var msg = new builder.Message(session).attachments([card]);
        session.send(msg);
        session.send('Hi I am the bot that dennis built.');
...
bot.dialog('/weather', [
    function (session, args) {
        session.endDialog('The weather in %s is 71 degrees and raining.', _.get(args, 'data'));
    }
]);
...
bot.dialog('/excuse', [
    function (session) {
        session.endDialog('Thanks but, ' + excuse.get());
    }
]);
...
bot.dialog('/gif', [
    function (session) {
        session.send('Sending GIFs works too!');
        var msg = new builder.Message(session)
            .attachments([{
                contentType: 'image/gif',
                contentUrl: 'https://media.giphy.com/media/5GoVLqeAOo6PK/giphy.gif'
            }]);
        session.endDialog(msg);
    }
]);
        ...
        var msg = new builder.Message(session)
            .attachments([
                new builder.AnimationCard(session)
                    ...
                    .media([
                        { profile: 'chimps', url: 'https://media.giphy.com/media/26tPo9rksWnfPo4HS/giphy.gif'}
                    ])
            ]);
        session.endDialog(msg);
    }
